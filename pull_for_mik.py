...
def just_so_i_can_call_it(alias):
    ...
    if 'Collection_TotalRecs.xml' not in os.listdir(alias_dir):
        print('pulling coll total recs')
        total_recs_data = p.retrieve_collection_total_recs(alias)
        p.write_xml_to_file(total_recs_data, alias, 'Collection_TotalRecs')
        total_recs_etree = etree.parse(total_recs_data)
    else:
        total_recs_etree = etree.parse("{}/Collection_TotalRecs.xml".format(alias_dir))

    if 'Collection_Fields.xml' not in os.listdir(alias_dir):
        print('pulling coll fields')
    ...
    # collection_fields_etree = etree.parse(collection_fields)
    ...
    for num in range(groups_of_1024):
        starting_pointer = (num * 1024) + 1                         # why the heavens does contentdm start counting at 1?
                                                                    # also, why can't cdm count above 10000??
        if 'Elems_in_Collection_{}.xml'.format(starting_pointer) not in os.listdir('{}/Collections/{}/'.format(os.getcwd(), alias)):
            ...
            print('pulling elems in collection {}'.format(starting_pointer))
        ...
        #     if item_etree.find('object') is not None:  # "find" is contentdm's abbr for 'contentdm file name'
        #         print('{}.{}'.format(pointer, filetype))
        #         if filetype not in ('cpd'):
        #             if '{}.{}'.format(pointer, filetype) not in os.listdir('{}/Collections/{}'.format(os.getcwd(), alias)):
        #                 binary = p.retrieve_binaries(alias, pointer, "something")
        #                 p.write_binary_to_file(binary, alias, pointer, filetype)
        #         else:
        #             xmldata = p.retrieve_item_metadata(alias, pointer)
        #             xml_text = bytes(bytearray(xmldata, encoding='utf-8'))
        #             local_etree = etree.fromstring(xml_text)
...
if __name__ == '__main__':
    ...
    # for coll in ('LSU_JJA', 'p16313coll54', 'LSU_HPL', 'p120701coll27', 'p16313coll21', 'p16313coll48', 'HPL', 'p15140coll16', 'CMPRT'):
    #     just_so_i_can_call_it(coll)
    ...
    if 'Collections_List.xml' not in os.listdir('{}/Collections/'.format(os.getcwd())):
