...
def just_so_i_can_call_it(alias):
    ...
    if not os.path.isfile('{}/Collection_Fields.json'.format(alias)):
    ...
    for num in range(groups_of_100):
        ...
        if not os.path.isfile('Cached_Cdm_files/{}/Elems_in_Collection_{}.xml'.format(alias, starting_pointer)):
        ...
        if not os.path.isfile('Cached_Cdm_files/{}/Elems_in_Collection_{}.json'.format(alias, starting_pointer)):
            fields_to_retrieve = ['source', 'dmrecord', 'dmimage', 'find']
        ...
        for pointer, filetype in pointers_filetypes:
            ...
            if filetype != 'cpd':
                ...
                if not os.path.isfile('Cached_Cdm_files/{}/Cpd/{}.json'.format(alias, pointer)):
                    item_json = p.retrieve_item_metadata(alias, pointer, 'json')
                    p.write_json_to_file(item_json, '{}/Cpd'.format(alias), pointer)

                if not os.path.isfile('Cached_Cdm_files/{}/Cpd/{}.xml'.format(alias, pointer)):
                    item_xml = p.retrieve_item_metadata(alias, pointer, 'xml')
                    p.write_xml_to_file(item_xml, '{}/Cpd'.format(alias), pointer)

                item_xml_filepath = '{}/Cached_Cdm_files/{}/Cpd/{}.xml'.format(os.getcwd(), alias, pointer)
                item_etree = etree.parse(item_xml_filepath)

                if not os.path.isfile('Cached_Cdm_files/{}/Cpd/{}_cpd.xml'.format(alias, pointer)):
